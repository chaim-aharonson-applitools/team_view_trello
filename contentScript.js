(function () {
    class LocalstorageHandler {
        ...
        set(itemName, value) {
            ...
            this[itemName] = value;
            localStorage.setItem(this.localstorage_name, JSON.stringify(this.lsObject));
        }
        _get(itemName) {
    ...
    let users, selected = [], viewMode = true;
        ...
        if (dom && dom.length){
            ...
            Array.from(dom).map(item => {
                ...
                Array.from(item.querySelectorAll(`.member`)).map(member => {
                    const elt = member.querySelector('.member-avatar') || member.querySelector('.member-initials');
    ...
    function addCssToPage(additionalCss = ''){
        const css = '#board .list-card.my-trello { outline-offset: -2px; outline: 2px dashed red } #board .list-card.my-trello .list-card-title{background-color: #fff2cd}' + additionalCss;
    ...
    chrome.runtime.onMessage.addListener(
        function (request, sender, sendResponse) {
            ...
            if (request.action === 'getUsersData'){
                sendResponse({ users: users || [], viewMode: viewMode, selected: selected });
